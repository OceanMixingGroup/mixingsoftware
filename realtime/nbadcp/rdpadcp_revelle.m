function [adcp,cfg,ens]=rdpadcp(name,num_av,nens,cfg,progyear);
...
for k=1:n,
  ...
  if cfg.adcp_type==0,
    adcp.mtime(k)=datenum(progyear,1,1)+adcp.time(k)/86400 - datenum(23,0,0);
  else
      %% Do this for BB!
  end;
...
%   adcp.east_vel(:,k)    =nmedian((ens.east_vel -ens.bt_east_vel(ol,:)*0) ,vels(1),2);
%   adcp.north_vel(:,k)   =nmedian((ens.north_vel-ens.bt_north_vel(ol,:)*0),vels(1),2);
%   adcp.vert_vel(:,k)    =nmedian((ens.vert_vel -ens.bt_vert_vel(ol,:)*0) ,vels(2),2);
%   adcp.error_vel(:,k)   =nmedian((ens.error_vel-ens.bt_error_vel(ol,:)*0),vels(3),2);
