function ppod = readppod6(fullfilename)
% load a High-speed PPOD data file and convert only 1 hz pressures
...
% the ppod structure is built---but then discarded as we save only pressure
    ...
    fseek(fid,headeroffset+28,'bof');
        ...
        % on the HSPPOD, the Armtime is phase locked to the DS-3232
        % so we return the time as dstime for backward compatibiity
        ppod.dstime=(Armtime./86400+dateoffset);
