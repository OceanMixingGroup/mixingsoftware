...
clear data
global head
q.memory=10000; % This is the number of lines to load in as a unit
                                  % Does not affect the result, only the
                                  % memory utilization.
...
Data=[];
q.count=64*q.memory;
while q.count==q.memory*64
  [temp q.count]=fread(fid,[64,q.memory],'uint8');
  Data=[Data  uint8(temp)];
end
toc
% Data=((Data/32768)-1)*4.5;
 ...
 for i=1:head.num_sensors
   if head.modulas(i,1)==1
     eval(['data.' deblank(head.sensor_name(i,:)) '(:,1:2)=Data(' ...
	   num2str(2*head.offset(i)+1) ':' num2str(2*head.offset(i)+2) ',:)'';']);
   else
     eval(['data.' deblank(head.sensor_name(i,:)) '=reshape(Data(' num2str(2*head.offset(i)+1) ':'...
	   num2str(floor(64/head.modulas(i,1))) ':64,:),' num2str(length(Data) ...
	   *head.modulas(i,1)) ',1);']);
     eval(['data.' deblank(head.sensor_name(i,:)) '(:,2)=reshape(Data(' num2str(2*head.offset(i)+2) ':'...
	   num2str(floor(64/head.modulas(i,1))) ':64,:),' num2str(length(Data) ...
	   *head.modulas(i,1)) ',1);']);
   end
