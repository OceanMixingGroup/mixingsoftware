function nadcp=beam2earth_sentinel5(adcp);
...
hb=waitbar(0,'transforming to earth');
for ii=1:size(adcp.east_vel,2)
waitbar(ii/size(adcp.east_vel,2),hb)
    beam=[adcp.east_vel(:,ii) adcp.north_vel(:,ii) adcp.vert_vel(:,ii) adcp.error_vel(:,ii)]*1000;
    head=adcp.heading(ii);
    HeadingOffset=0;
    pitch=adcp.pitch(ii);
    roll=adcp.roll(ii);
    ssnd=1500; %adcp.speedofsound(ii);
    ECssnd=1500; % speed of sound assumption for EC command
    beams_up=strcmp('up',adcp(1).config.orientation);
    xfreq=500; % transmitter frequency
    convex = -1; % AP change 1 to -1 08/30/15
    sensor_config=1;
    BeamAngle = 25; % AP changed from 25
    % check inputs
...
%           disp('wowza!') % AP debug
    ...
    nadcp.vel1(:,ii)=earth(:,1)*0.001;
    nadcp.vel2(:,ii)=earth(:,2)*0.001;
    nadcp.vel3(:,ii)=earth(:,3)*0.001;
    nadcp.vel4(:,ii)=earth(:,4)*0.001;
    
    %~ AP for debugging
%     figure(1);clf
%     plot(nadcp.vel1(:,ii),1:size(adcp.east_vel,1))
%     title(num2str(ii))
%     axis ij
%     pause(0.1)
    %~
