...
% shortwave radiation sw [W/m^2] (negative if downward),
...
%
% OUTPUT
% Jb: total buoyancy flux [m^2/s^3] (positive upward)
% Jb_ec: buoyancy flux due to evaporation/condensation
% Jb_p: buoyancy flux due to precipitation
% Jb_qs: buoyancy flux due to sensible heat flux
% Jb_sw: buoyancy flux due to shortwave radiation
% Jb_lw: buoyancy flux due to longwave radiation
% 
% formula is based on Dorrestein (1978) (JPO, 9, 1979, p.229-231)
% M=s*beta*(P-E)-alpha/c*(L*E+Q)+alpha*deltaT*P;
% Jb=-g/rho0*M
% $Revision: 1.3 $ $Date: 2009/04/24 16:07:03 $ $Author: aperlin $	
% Originally A. Perlin 
...
% eq. of statewith fixed temperature and pressure
beta=0.7;
...
Jb=-g/rho0*M; % total buoyancy flux
Jb_ec=-g/rho0*M_ec; % buoyancy flux due to evaporation/condensation
Jb_p=-g/rho0*M_p; % buoyancy flux due to precipitation
Jb_qs=-g/rho0*M_qs; % sensible buoyancy flux
Jb_sw=-g/rho0*M_sw; % buoyancy flux due to shortwave radiation
Jb_lw=-g/rho0*M_lw; % buoyancy flux due to longwave radiation
if check==1;
    Jb=Jb';Jb_ec=Jb_ec';Jb_p=Jb_p';Jb_qs=Jb_qs';Jb_sw=Jb_sw';Jb_lw=Jb_lw';
end
