function [d fromdir]=dirdiff(frompath,topath,ignoresize,ignoredate);
...
for i=1:length(fromdir);
  if ~(fromdir(i).name(1)=='.');
    ...
    if fromdir(i).bytes
      ...
      if ~isempty(dd)
	if ignoresize
          dd.bytes=fromdir(i).bytes;
        end;
        if ignoredate
          dd.date = fromdir(i).date;
        end;
        
        if dd.bytes==fromdir(i).bytes & datenum(dd.date)== ...
		    datenum(fromdir(i).date)
