function calibrate_w_stat(highpass_length,lowpass_freq,varargin)
% CALIBRATE_W_STAT is same as CALIBRATE_W except that is adjusts
% calibration to account for static pitot calibration, rather than
% our old dynamic method. The difference is just a factor of 2
% NOTE: head.coef.W should give P in dynes / cm2
...
%% [bl,al]=cheby2(5,40,lowpass_freq/fn_w);% lpf butter coeffs.in=filtfilt(bl,al,in);
...
out = (in ./ (rho .* spd .* sp)) * (1/100); % Note: 1/100 converts to in m/s from cm/s
...
% hold on
  ...
  %% [bl,al]=cheby2(5,40,lowpass_freq/fn_az);% lpf cheby coeffs.  azz=filtfilt(bl,al,azz);
