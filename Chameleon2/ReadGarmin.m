 ...
 try
    ...
    GPS_in = struct();
        ...
        if strcmp(sentence(1:6),'$GPRMC') 
            [prefixRMC,timeRMC,ActiveRMC,lat,latdirRMC,lon,londirRMC,spdKnots,AngleDeg,DateRMC] = strread(sentence, '%s%f%s%f%s%f%s%f%f%f', 1, 'delimiter', ',');
  ...
  % or the sentence wasn't long enough to fill in lat and lon
      ...
      disp('Error Reading Data! Check Unit')
            ...
            GPS_in.timeRMC = timeRMC;
            GPS_in.lat = lat;
            GPS_in.latdirRMC = latdirRMC;
            GPS_in.lon = lon;
            GPS_in.londirRMC = londirRMC;
            GPS_in.speed = spdKnots;
