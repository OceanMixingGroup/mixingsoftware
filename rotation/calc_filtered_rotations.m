function cal=calc_filtered_rotations(cal,head)
...
number=head.samplerate(head.sensor_index.R1)*600; % 10 minutes
len=length(cal.RX);
...
for i=1:length(names1)
    ...
    in=[fin in  bin];
    in=detrend(gappy_filt(head.samplerate(head.sensor_index.R1),{'h0.005'},4,in,1,1,20));
    in=gappy_filt(head.samplerate(head.sensor_index.R1),{'l1'},4,in,1,1,20);
