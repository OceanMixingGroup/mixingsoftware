function [eps, slope, varargout] = fit_longitudinal_kolmogorov_test(time, u, f_range, varargin)
...
%_____________________calculate spectrum______________________
   ...
   if isnan(u_m)
      ...
      [spec,k] = gappy_psd( u, floor(Nw/2), ks, 5);
   end
...
if do_plot
   figure
      ...
      t = text_corner(ax(a), ['$\langle{u}\rangle = $' num2str(u_m) 'm s$^{-1}$'], 3);
