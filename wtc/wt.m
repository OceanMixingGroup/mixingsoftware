...
%
% USAGE: [wave,period,scale,coi,sig95]=cwt(d[,settings])
%
% 
...
if isempty(Args.J1)
    if isempty(Args.MaxScale)
        Args.MaxScale=(n*.17)*2; %automaxscale
    end
...
if Args.MakeFigure
    ...
    [signif,fft_theor] = wave_signif(1.0,dt,scale,0,lag1,-1,-1,Args.Mother);
...
varargout={wave,period,scale,coi};
