function [chi,epsil,k,spec,k_kraich,spec_kraich,stats]=get_chipod_chi(freq,tp_power,fspd,nu,tdif,dTdz,varargin)
% function [chi,epsil,k,spec,k_kraich,spec_kraich]=get_chipod_chi(freq,tp_power,fspd,nu,tdif,dTdz,varargin)
% to compute chi and epsilon from chipod data.  
% Required arguments are freq, the frequencies of tp_spec (corrected 
% temperature gradient [dT/dt] spectrum), fallspd (in m/s), nu (viscosity),
% tdif (thermal diffusivity), dTdz (a scalar vertical gradient), 
% the head structure, and this_sensor (the sensor name
