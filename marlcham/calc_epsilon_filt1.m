function [epsilon_real,k,spec,k_nas,spec_nas,irange]=calc_epsilon_filt1(s1_power,fre,fallspd,nfft,nu,head_index_num,filters,k_start,k_stop,kstop_coef,head)
% CALC_EPSILON calculates epsilon from shear data.  
% Same as calc_epsilon_filt_gen, but uses computed shear spectra instead 
% of computing it in the function
% function CALC_EPSILON(S1_POWER,FRE,FSPD,NFFT,NU,SENSOR_INDEX,FILTERS) calculates epsilon
% from shear data.  S1_POWER, FRE and FSPD are required; NFFT, NU and SENSOR_INDEX are
...
% S1_POWER is power shear spectra density, corrected for accelerometer 
% and computer in average_data_gen2, FRE is the corresponding frequency
%
...
%
% $Revision: 1.6 $ $Date: 2011/05/17 17:10:43 $ $Author: aperlin $	


% Originally A. Perlin, 2002


if any(isnan(s1_power)) || any(isinf(s1_power))
  epsilon_real=NaN;
  fre=NaN;
  sss=NaN;
  unfreq=NaN;
...
% 8 pole anti-aliasing filters for shear circuits in Chameleon are used since 2004
% filt_ord=4;
filt_ord=8;
...
cutfreq=[];
  ...
  for j=1:nfilts
    ...
    if type=='l'
      % LOWPASS
      cutfreq(j*2-1)=str2double(filts(2:length(filts)));
      cutfreq(j*2)=Inf;
    elseif type=='h'
      % HIGHPASS
      cutfreq(j*2-1)=0;
      cutfreq(j*2)=str2double(filts(2:length(filts)));
    elseif type=='n'
      % NOTCH
      position=find(filts=='-');
      cutfreq(j*2-1)=str2double(filts(2:(position-1)));
      cutfreq(j*2)=str2double(filts((position+1):length(filts)));
    end
  end
...
% calculate epsilon(s1)
  ...
  % This adjustment filters has already been done by calc_shearspec,
  % which is what is used by average_data_gen1 to produce the input
  % s1_power to this function.
%   sss=invert_filt(fre,spa_cor(s1_power,fre,fspd),filt_ord,fcutoff);
  
  sss=s1_power; % filter/spa correction have been already performed in calc_shearspec.m
%  loglog(fre,sss,'r');
%  hold on
  ...
  if epsilon_real<1e-10
    epsilon_real=1e-10;
  elseif epsilon_real>.1
    epsilon_real=.1;
  end
