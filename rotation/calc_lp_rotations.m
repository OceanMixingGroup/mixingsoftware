function cal=calc_lp_rotations(cal,head,number)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin<3
  number=head.samplerate(head.sensor_index.AX)*300; % 5 minutes
end
len=length(cal.AX);
...
sr=head.samplerate(head.sensor_index.AX); % sample rate
smpl=1:sr/head.slow_samp_rate:len;
if length(temp.CMP)>length(smpl)
    temp.CMP=temp.CMP(1:end-1);
elseif length(temp.CMP)<length(smpl)
    temp.CMP=[temp.CMP temp.CMP(end)];
end
temp.CMPy=interp1(smpl,temp.CMP,1:len);
...
for i=1:length(names1)
    ...
    in=[fin in  bin];
    in=gappy_filt(sr,{'l0.01'},4,in,1,1,20);
    in=gappy_filt(sr,{'l0.01'},4,in,1,1,20);
...
% g=mean(cal.AZlp);
g=9.81;
cal.rlp=real(asin(cal.AYlp/g)*180/pi);
cal.plp=real(asin(cal.AXlp./cosd(cal.rlp)/(-g))*180/pi);
