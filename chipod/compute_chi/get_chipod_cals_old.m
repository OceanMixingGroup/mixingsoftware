...
% for every cruise!!!  
%   $Revision: 1.7 $  $Date: 2010/03/17 18:32:31 $
clear vel disp
...
[cur]=get_current_spd(dpath,ts,tf,depth);
% interpolate water speed to Chipod time
cal.cur_u=interp1(cur.curtime,cur.u,cal.time,'spline');
cal.cur_v=interp1(cur.curtime,cur.v,cal.time,'spline');
% cal.cur_w=interp1(cur.curtime,cur.w,cal.time,'spline');
current_spd=interp1(cur.curtime,cur.spd,cal.time,'spline');
cal.cur_ufast=interp1(1:length(cal.time),cal.cur_u,[1:length(cal.AX)]/12,'linear','extrap');
cal.cur_vfast=interp1(1:length(cal.time),cal.cur_v,[1:length(cal.AX)]/12,'linear','extrap');
% cal.cur_wfast=interp1(1:length(cal.time),cal.cur_w,[1:length(cal.AX)]/12,'linear','extrap');
cal.curspd=interp1(1:length(cal.time),current_spd,[1:length(cal.AX)]/12,'linear','extrap');
if size(cal.cur_ufast,1)~=size(cal.AX,1)
    cal.cur_ufast=cal.cur_ufast';
    cal.cur_vfast=cal.cur_vfast';
    cal.curspd=cal.curspd';
end
cal.time_fast=interp1(1:length(cal.time),cal.time,[1:length(cal.AX)]/12,'linear','extrap');
if isfield(cal,'RX') && ~all(isnan(cal.CMP))
    % filter rate sensor data and get high-freq rotations
    cal=calc_filtered_rotations(cal);
    % calculate low-freq rotations from accelerometer data
    cal=calc_lp_rotations(cal);
    ...
    [cal.cur_xfast cal.cur_yfast cal.cur_zfast] = translatevectorrpy([-r -p -y],[curu curv curw],'degrees');
    if size(cal.cur_xfast,1)~=size(cal.AX,1)
        cal.cur_xfast=cal.cur_xfast';
        cal.cur_yfast=cal.cur_yfast';
        cal.cur_zfast=cal.cur_zfast';
    end
    ...
    dyawdt=gradient(cal.yaw*pi/180,1/120);dyawdt=deglitch(dyawdt,30,2);dyawdt=fillgap(dyawdt,1);
    ...
    cal.vely=vel.y;
    cal.velz=vel.z;
    cal.fspd=sqrt((cal.velx-cal.cur_xfast).^2+(cal.vely-cal.cur_yfast+cal.r1omega).^2+...
        (cal.velz-cal.cur_zfast).^2);
...
elseif ~all(isnan(cal.CMP))
    ...
    [time itime]=unique(cal.time_fast(1:120:end)-1/86400);
    cmp=exp(sqrt(-1)*cal.CMP(itime)*pi/180);
    cmpu=real(cmp); cmpv=imag(cmp);
    cmpu=interp1(time,cmpu,cal.time_fast,'linear','extrap');
    cmpv=interp1(time,cmpv,cal.time_fast,'linear','extrap');
    CMP_fast=atan2(cmpv,cmpu)*180/pi;
    cal.cur_xfast=cal.cur_ufast.*sind(CMP_fast)+cal.cur_vfast.*cosd(CMP_fast);
    cal.cur_yfast=-cal.cur_ufast.*cosd(CMP_fast)+cal.cur_vfast.*sind(CMP_fast);
    % calculate tangential velocity of the sensor tip minus tangential
    % velocity of the accelerometer (part of the accelerometer signal)
    r1tip=1.818*2.54/100;% distance from cable to shorter sensor tip
    ra=3.3*0.0254;% distance from cable to accelerometer package
    dyawdt=gradient(-CMP_fast*pi/180,1/120);dyawdt=deglitch(dyawdt,30,2);dyawdt=fillgap(dyawdt,1);
    ...
    cal.dispx=disp.x;
    % calculate fallspeed
    cal.fspd=sqrt((cal.velx-cal.cur_xfast).^2+(cal.vely-cal.cur_yfast+cal.r1omega).^2+cal.velz.^2);
    if isfield(data,'W1')
        r2tip=3.76*2.54/100;% distance from cable to longer sensor tip (3 pitot unit)
        cal.r1omega_3P=(r2tip-ra)*dyawdt;
        cal.fspd_3P=sqrt((cal.velx-cal.cur_xfast).^2+(cal.vely-cal.cur_yfast+cal.r1omega_3P).^2+cal.velz.^2);
    end
else
    [disp,vel]=integrate_acc(cal,head,hpf_cutoff);
    cal.velz=vel.z;
