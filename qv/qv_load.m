...
[data,head]=raw_load([head.pathname raw_name]);
if 1==0
...
if any(strcmp('P',fieldnames(data)))
  p=qv_calp(data.P,head.coef.P)*0.689476;
...
%    data.fspd=100*[0 ; diff(filtfilt(q.butter(1:3),q.butter(4:6),(p8)))]*slow_samp_rate*8;
    data.fspd=100*[0 ; diff(filtfilt(q.butter(1:3),q.butter(4:6),(p32)))]*slow_samp_rate*32;
else
...
if isfield(data,'P')
  str='P';
elseif isfield(data,'P1')
  str='P1';
elseif isfield(data,'P2')
  str='P2';
end
eval(['y_axis=(1:(length(data.' str ')/irep.' str '))''/slow_samp_rate;']);
q.yaxis_label='time (sec)';
global mins maxs
mins.y=min(y_axis);
maxs.y=max(y_axis);
y_axis8=interp8(y_axis,8);
y_axis32=interp8(y_axis,32);
