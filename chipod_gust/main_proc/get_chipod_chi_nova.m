function [chi,epsil,k,spec,k_kraich,spec_kraich,stats]=...
    get_chipod_chi_nova(freq,tp_power,fspd,nu,tdif,dTdz,varargin)
%
% function [chi,epsil,k,spec,k_kraich,spec_kraich,stats]=...
...
spec_k=tp_power/fspd/2/pi;% If tp_power is power of dT/dt, than our units are 
...
for b=1:n_iterations
    ...
    kb=(ks*sqrt(nu/tdif));
    k_end = kb/2;
    f_start= k_start * fspd/(2*pi);
    f_stop = k_end * fspd/(2*pi);
    if f_stop>fmax;
        f_stop=fmax;
    end
    if max(freq)<f_stop
    ...
    k_stop = 2*pi/fspd*f_stop;
    k_start = 2*pi/fspd*f_start;
    %  freq=tp_freq';
    chi_part= 6*tdif* integrate(k_start,k_stop,k',spec_k','inclusive');
    chi=chi_part;
    ...
    while abs(chi_part/chi_test-1)>.01
        count=count+1;
        b_spec= kraichnan(nu,b_freq/fspd*2*pi,kb,tdif,chi,q);
        chi_part= 6*tdif* integrate(k_start,k_stop,k',spec_k','inclusive');
        chi_test=6*tdif*integrate(k_start,k_stop,b_freq/fspd*2*pi,b_spec,'inclusive');
        chi=chi*chi_part/chi_test;
        % Now get a new estimate of kb and f_stop:
        epsilon=nsqr*chi/(2*gamma*dTdz^2);
        kb = (((epsilon/(nu^3))^.25 ))*sqrt(nu/tdif);
        k_stop=(kb/2);
        if f_stop>fmax;
            f_stop=fmax;
        end
...
%                 disp('\chi is set to NaN')
    ...
    k_kraich=b_freq/fspd*2*pi;
    spec_kraich=b_spec;% to convert from K/[m^2*Hz] to K/[m^2*cpm]
    spec=spec_k;% to convert from K/[m^2*Hz] to K/[m^2*cpm]
