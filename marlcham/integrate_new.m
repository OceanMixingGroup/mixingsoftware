function [integrate]=integrate(kmin,kmax,freqs,spec,flg)
...
if nargin==4 || strcmp(flg,'exclusive')
  indices=find(freqs>kmin & freqs<kmax);
elseif strcmp(flg,'inclusive')
  indices=find(freqs>=kmin & freqs<=kmax);
end
if isempty(indices)
  integrate=NaN;
...
%   Modified by A. Perlin 14 Sep 2012:
%   The result is identical to the line above, but computationa is faster:
%   Trapezoid sum computed with vector-matrix multiply (az in trapz).
integrate=diff(newfreqs,1,1).'*(newspec(1:maxnum+1)+newspec(2:maxnum+2))/2;
