function [cal head]=mg_calibrate(series,method,filters,data,head,cal)
% function cal=mg_calibrate(SERIES,METHOD,FILTERS,DATA,HEAD,CAL) 
% calibrates SERIES using METHOD.  
...
series=upper(series);
inser=data.(series);
ireps=head.irep.(series);
filt_ord=4;

% for tp and ucp, the diferentiated series may be calibrated when the
...
%  Calibrate FALLSPD
  if exist('a','var')
    eval(['[cal.FALLSPD head]=calibrate_' m '(inser,head.coef.' series ',ireps,a,b,head);'])
  else
