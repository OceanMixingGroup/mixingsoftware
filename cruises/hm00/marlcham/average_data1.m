...
% this function is modified for HOME2000 CHAMELEON data computations
  ...
  if strncmp(active,'EPSILON',7)
    ...
    for n=1:nmax
      ...
      nu=sw_visc(sal(n),temp(n),pres(n)); 
    	...
    	% eval(['[avg.EPSILON' prb '(n),SHR_K,SHR_SPEC] =calc_epsilon(cal.S' prb '(1+(min_ind(n)-1)*head.irep.S' prb ...
	 	%	':max_ind(n)*head.irep.S' prb '),avg.FALLSPD(n),nfft,sw_visc(sal(n),temp(n),pres(n)),' ...
      % 	'head.sensor_index.S' prb ');'])
      %#############################################################################
      %the following is calculation of epsilon for HOME'00 Chameleon time series #1
      if prb=='1'
         eps=avg.EPSILON1(n);
      elseif prb=='2'
         eps=avg.EPSILON2(n);
      ...
      if eps>1.e-7
         eval(['avg.EPSILON' prb '(n)=calc_epsilon_filter2(cal.S' prb '(1+(min_ind(n)-1)*head.irep.S' prb ...
               ':max_ind(n)*head.irep.S' prb '),avg.FALLSPD(n),nfft,nu,' 'head.sensor_index.S' prb ',{},0.3);'])
      elseif eps>5.e-8
         eval(['avg.EPSILON' prb '(n)=calc_epsilon_filter2(cal.S' prb '(1+(min_ind(n)-1)*head.irep.S' prb ...
               ':max_ind(n)*head.irep.S' prb '),avg.FALLSPD(n),nfft,nu,' 'head.sensor_index.S' prb...
               ',{''n6-7.5'',''n10-12.5'',''n21-22.5''},0.36);'])
      elseif eps>1.5e-8
         eval(['avg.EPSILON' prb '(n)=calc_epsilon_filter2(cal.S' prb '(1+(min_ind(n)-1)*head.irep.S' prb ...
               ':max_ind(n)*head.irep.S' prb '),avg.FALLSPD(n),nfft,nu,' 'head.sensor_index.S' prb...
               ',{''n6.5-9.5'',''n11-13'',''n20.5-23.5''},0.42);'])
      else
         eval(['avg.EPSILON' prb '(n)=calc_epsilon_filter2(cal.S' prb '(1+(min_ind(n)-1)*head.irep.S' prb ...
               ':max_ind(n)*head.irep.S' prb '),avg.FALLSPD(n),nfft,nu,' 'head.sensor_index.S' prb...
               ',{''n6-8.5'',''n11-12.5'',''n20.5-24''});'])
      end
      %#############################################################################
       ...
       '*head.irep.AX_TILT:max_ind(n)*head.irep.AX_TILT),nfft,head.slow_samp_rate*head.irep.AX_TILT);']);
 		...
 		%-------------------------------------------------
		% f_int1 = [10 20];	f_int2 = [20 30];	fact = 2; 
 
 		% clear Q
    	% Q = check_spec(AX_power,AX_fre,avg.FALLSPD(n),f_int1,f_int2,fact);
      % Q = 0;
      % Q
      
    	% if Q==1
      % 	eval(['avg.EPSILON' prb '(n)=calc_epsilon_filter(cal.S' prb '(1+(min_ind(n)-1)*head.irep.S' prb ...
	 	%	':max_ind(n)*head.irep.S' prb '),avg.FALLSPD(n),nfft,sw_visc(sal(n),temp(n),pres(n)),' ...
      % 	'head.sensor_index.S' prb ',' '''n20-30''' ');'])
      % end
      %---------------------------------------
