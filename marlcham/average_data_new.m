function avg=average_data_new(series,axs,varargin)
% function AVG=AVERAGE_DATA(SERIES,AXS,PROPERTY1,VALUE1,PROPERTY2,VALUE2,...)
% Same as average_data_gen, but implements Jody's correction
% for calculating epsilon (correction for accelerometer)
...
%
% AXS is a series of axelerometers in order, corresponding to shea sensors, i.e.
% {'AX','AY'} means that 'AX' is ligned with 'S1', and 'AY' is aligned with 'S2'
% it is done to use Jody's correction for shear spectra (subtract accelerometer
% spectra from it)
%
...
%       
      ...
      sin = [1+(min_ind(n)-1)*irep:max_ind(n)*irep];
%       Correcting with accelerometer spectra shear spectra (Jody's routine)
      ax = 9.81*makelen(getfield(cal,axs{str2num(prb)}),length(cal.S1))';
      p = fast_cohere(Sdata(sin),ax(sin),nfft,samplerate,[1 1]);
      ss = (p.xx-abs(p.xy./p.yy).^2.*p.yy)';
      f=p.f';
      
      eval(['set_filters_' q.script.prefix '_new']);
