...
%Author Unknown - Ocean Mixing Group
...
for ii=1:head.num_sensors
 ...
 % F=fread(fid,10,'char');
  ...
  head.coefficients(ii,5)=0;
  head.coefficients(3,1)=0.1;
  head.coefficients(3,2:5)=0;
  head.coefficients(4,:)=0;
  head.coefficients(5,2:5)=0;
  head.coefficients(7,2:5)=0;
  head.coefficients(15,:)=0;
  head.coefficients(16,:)=0;
  head.coefficients(4,:)=0;
  head.coefficients(6,:)=0;
  head.coefficients(8,4)=0;
  head.coefficients(14,1:2)=[2.6572 -1.0781]; %AZ
  head.coefficients(12,1:2)=[-2.55892 1.08108]; %AY
  head.coefficients(10,1:2)=[2.57483 -1.05764]; %AX
...
%F=fread(fid,20,'char');
head.starttime=char(head.starttime);

fwrite(fid,head.starttime(1:20),'char');
%F=fread(fid,20,'char');
head.endtime=char(head.endtime);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function head = Getsaildata(head)
