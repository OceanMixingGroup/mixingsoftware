function Write_Latex_Notes(the_project)
%~~~~~~~~~~~~~~~~~~~~~~~~~~
%
% Write_Latex_Notes.m
%
...
fprintf(fileID,['{' fullfile(cruise_path,the_project) '/}\n'])
fprintf(fileID,['{' fullfile(cruise_path,the_project,'figures') '/}\n'])
fprintf(fileID,['{' fullfile(cruise_path,the_project,'figures','XC') '/}\n'])

% loop through and add path to each SN here
for iSN=1:length(ChiInfo.SNs)
%    fprintf(fileID,['{/Users/Andy/Cruises_Research/ChiPod/' the_project '/Data/proc/Chipod/SN' ChiInfo.SNs{iSN} '/figures/}\n'])
    fprintf(fileID,['{' fullfile(cruise_path,the_project,'figures','proc',[ChiInfo.SNs{iSN}]) '/}\n'])
end
fprintf(fileID,'} \n\n')

%% Begin document

fprintf(fileID,[ '\\title{' the_project ' CTD-Chipod Notes} \n'])
...
fprintf(fileID, ['Notes on processing and analysis of CTD-chipod data collected during ' the_project ' cruise.. \n\n'])

fprintf(fileID, '\\begin{figure}[htbp] \n')
fprintf(fileID, ['\\includegraphics[width=38pc]{' the_project '_kml_map.jpg} \n'])
fprintf(fileID, ['\\caption{Map of CTD cast locations during cruise ' the_project '.} \n'])
...
fprintf(fileID, ['\\item \\verb+Load_chipod_paths_' the_project '.m+ \n'])
fprintf(fileID, '\\end{itemize} \n\n')

fprintf(fileID, 'Chipod deployment info is given in  \n')
fprintf(fileID, '\\begin{itemize} \n')
fprintf(fileID, ['\\item \\verb+Chipod_Deploy_Info_' the_project '+ \n'])
fprintf(fileID, '\\end{itemize} \n\n')
...
fprintf(fileID, '\\begin{itemize} \n')
fprintf(fileID, ['\\item \\verb+Process_CTD_hex_' the_project '.m+ \n'])
fprintf(fileID, '\\end{itemize} \n\n')
...
fprintf(fileID, '\\begin{itemize} \n')
fprintf(fileID, ['\\item \\verb+PlotChipodDataRaw_' the_project '.m+ \n'])
fprintf(fileID, '\\end{itemize} \n')
...
fprintf(fileID, '\\begin{itemize} \n')
fprintf(fileID, ['\\item \\verb+MakeCasts_CTDchipod_' the_project '.m+ \n'])
fprintf(fileID, ['\\item \\verb+DoChiCalc_' the_project '.m+ \n'])
fprintf(fileID, '\\end{itemize} \n\n')
...
fprintf(fileID, '\\begin{itemize} \n')
fprintf(fileID, ['\\item Data from all casts are combined into a single structure w/ \verb+MakeCombinedStruct_' the_project '.m+ \n'])
fprintf(fileID, '\\item  \n')
fprintf(fileID, '\\end{itemize} \n\n')
...
fprintf(fileID, '\\begin{figure}[htbp] \n\n')
fprintf(fileID, ['\\includegraphics[scale=1]{' the_project '_timeoffsets_all.png} \n'])
...
%* choose a cast at random for each one?
for iSN=1:length(ChiInfo.SNs)
    whSN = ChiInfo.SNs{iSN} ;
    fprintf(fileID, '\\begin{figure}[htbp] \n')
    fprintf(fileID, ['\\includegraphics[scale=0.7]{' whSN '_001_Fig1_RawChipodTS.png} \n'])
    fprintf(fileID, ['\\caption{Raw chipod data from ' whSN ' for a CTD cast.} \n'])
    fprintf(fileID, ['\\label{' whSN '_1} \n'])
    fprintf(fileID, '\\end{figure} \n\n')
...
fprintf(fileID, '\\begin{figure}[htbp] \n')
fprintf(fileID, ['\\includegraphics[scale=0.7]{' the_project '_ctd_t_s.png} \n'])
...
fprintf(fileID, '\\begin{figure}[htbp] \n')
fprintf(fileID, ['\\includegraphics[scale=0.7]{' the_project '_chi_AllSNs_Vslat.png} \n'])
fprintf(fileID, '\\caption{Plot of one variable from all chipods.} \n')
fprintf(fileID, '\\label{} \n')
fprintf(fileID, '\\end{figure} \n\n')

fprintf(fileID, '\\begin{figure}[htbp] \n')
fprintf(fileID, ['\\includegraphics[scale=0.7]{' the_project '_KT_AllSNs_Vslat.png} \n'])
fprintf(fileID, '\\caption{Plot of one variable from all chipods.} \n')
fprintf(fileID, '\\label{} \n')
fprintf(fileID, '\\end{figure} \n\n')
