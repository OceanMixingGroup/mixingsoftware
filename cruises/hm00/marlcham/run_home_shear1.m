% run_home_shear.m
% path(path,'\\Balt\rdata\work\home\marlin\proc\');
% path(path,'\\Balt\rdata\work\cal\');
clear all
path(path,'\\Balt\rdata\matlabR12\work\cal\');

path_raw = '\\Balt\rdata\HOME\Marlin\Marlin\';
path_save= 'c:\sasha\eps_mat\';
load c:\sasha\velocity_eq
load c:\sasha\sum_tcp
DIR = dir(path_raw);
a={DIR.name};
I = strmatch('hm00',a);
a = a(I);
...
for cast=3100:4613% 1:4614
    if cast~=[1:193,408:416,593:604,666,856:872,943,977:983,1881,1882,1914:1924,1995,2255,...
                2610,2614,2633,2821,2865,3394:3395,4287,4295:4296];
      file=sprintf('hm00%4.3f',cast/1000);
      if exist([path_raw file],'file')
      disp(cast)
      q.script.num=cast;
      clear global head data cal
      global data cal head 
      [data,head] = raw_load1(q);
      cali_hm00_shear;
      
      nfft=256;
      q.series={'ptime','ctime','stime','t1','c1','s','fallspd','epsilon1','epsilon2','epsilon3'};
      warning off
      binsize=2;%sec
