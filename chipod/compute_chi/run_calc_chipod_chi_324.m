function run_calc_chipod_chi(dpath,outpath,dpl,unit,ts,tf,dt,depth,...
    hpf_cutoff,do_noise,salinity,use_n2,time_offset,min_dTdz,time_offset_name)
% 
% function run_calc_chipod_chi(dpath,outpath,dpl,unit,ts,tf,dt,depth,...
...
for itime=1:niter
    ...
    [cal,data,head]=get_chipod_cals_324(dpath,dpl,unit,ts,tf,depth,...
        hpf_cutoff,use_n2,time_offset,time_offset_name);
    ...
    for ik=1:length(avg.time)
        ...
        if length(id)>5
            ...
            % rename N2 and dTdz if they exist
            if isfield(avg,'N2')
                Nsqr=avg.N2(ik);
            end
            if isfield(avg,'dTdz')
            ...
            if avg.fspd(ik) >= 0.04 && alpha1>0 && dTdz>min_dTdz
                ...
                if ~exist('Nsqr','var')
                    [chi,epsil,k,spec,k_kraich,spec_kraich,stats]=...
                        get_chipod_chi(freq,tp_power,fspd,nu1,tdif1,dTdz,...
