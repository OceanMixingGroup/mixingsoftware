...
cal.p=filtfilt(q.butter(1:3),q.butter(4:6),(qv_calp(ch(data.P),coef.P)*0.689476));
...
% ****************************************
% THERMISTORS
cal.t1=qv_calp(ch(data.T1),coef.T1);
...
% calibrate t-prime
cal.tp1=qv_caltp(temp,coef.TP,ch(data.T1),coef.T1,cal.fspd);
irep.TP1=irep.TP;
...
cal.t2=qv_calp(ch(data.T2),coef.T2);
cal.tp2=qv_caltp(ch(data.ZYNC),coef.ZYNC,ch(data.T2),coef.T2,cal.fspd);
irep.TP2=irep.ZYNC;

% ****************************************
% Thermocouple

[cal.tc,coef.T0]=qv_caltc(cal.t1,ch(data.T0),ch(data.ZQUARE),q.mini,q.maxi);
irep.TC=irep.T0;
[b,a]=butter(2,.2/2);
temp=filtfilt(b,a,data.SQUARE);
